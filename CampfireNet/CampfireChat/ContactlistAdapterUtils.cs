...
namespace CampfireChat {
   class ContactlistAdapter : RecyclerView.Adapter {
      public List<ContactEntry> FullEntries { get; private set; }
      public List<ContactEntry> CurrentEntries { get; set; }
      public event EventHandler<byte[]> ItemClick;

      private HashSet<int> selectedPositions;

      public ContactlistAdapter(List<ContactEntry> entries) {
         FullEntries = entries;
      ...
      {
         get { return CurrentEntries.Count; }
      }

      public void UpdateDataSet(List<ContactEntry> newData) {
         CurrentEntries = newData;
