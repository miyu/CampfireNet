				...
				foreach (var tuple in nodesToImport)
				{
					...
					if (node.Descendents == 0 && await localMerkleTree.GetNodeAsync(tuple.Item1).ConfigureAwait(false) == null)
					{
						...
						if (insertionResult.Item1 && isDataNode)
						{
							...
							{
								byte[] payloadWithoutMessageNum;
								if (decryptedPayload.Length > 4)
								{
									int newLength = decryptedPayload.Length - 4;
									payloadWithoutMessageNum = new byte[newLength];
									Buffer.BlockCopy(decryptedPayload, 4, payloadWithoutMessageNum, 0, newLength);
								}
								else
								{
									payloadWithoutMessageNum = decryptedPayload;
								}
								...
								BroadcastReceived?.Invoke(new BroadcastReceivedEventArgs(neighbor, new BroadcastMessage
								{
									...
									DecryptedPayload = payloadWithoutMessageNum,
									Dto = message
								}));
							}
						}
