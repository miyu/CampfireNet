...
namespace CampfireNet
{
	public class CampfireNetClient
	{
		...
		{
			var orderedMessage = new byte[payload.Length + 4];
			var messageNumBytes = BitConverter.GetBytes(messageNum);
			Buffer.BlockCopy(messageNumBytes, 0, orderedMessage, 0, 4);
			Buffer.BlockCopy(payload, 0, orderedMessage, 4, payload.Length);
			var messageDto = identity.EncodePacket(orderedMessage, null);
